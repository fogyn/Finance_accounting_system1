...
public class CepNameDoxod extends AbsBaseCepochka{
    @Override
    ...
    private boolean canHandle(ElementImpl element, Ars ars, String type, String n, BulIml bulsUp){
        ...
        if(type.contains("новый")){
            name = getMfc().choiseNameDoxod("Введите имя "+n, ars);
        }
        else{
            name =  getMupdt().choiseNameDoxod(ars,"Отредактируйте имя "+n+" , если требуется");
        }
        if(type.contains("новый") ){
